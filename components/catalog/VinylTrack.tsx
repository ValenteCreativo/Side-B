...
export function VinylTrack({ id, title, artist, price, audioUrl, imageUrl }: VinylTrackProps) {
    ...
    return (
        <div className="group relative w-full aspect-square bg-card border border-border hover:border-foreground transition-colors duration-300">
            ...
            <div className="absolute inset-0 bg-card z-10 flex flex-col justify-between p-6 border-r border-border group-hover:translate-x-[-10%] transition-transform duration-500">
                <div className="relative w-full aspect-square mb-4 overflow-hidden bg-secondary">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-background/80">
                        <button
                            onClick={handlePlay}
                            className="w-12 h-12 flex items-center justify-center rounded-full border border-foreground hover:bg-foreground hover:text-background transition-colors"
                        >
