        ...
        <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center relative z-10">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Link href="/studio">
                <Button variant="outline" size="lg" className="rounded-sm text-lg px-8 py-6 hover-bronze hover:text-bronze">
                  ENTER STUDIO
                </Button>
              </Link>
            </div>
      ...
      <footer className="py-12 bg-zinc-50 dark:bg-zinc-950 border-t border-zinc-200 dark:border-zinc-800">
        <div className="container mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex gap-8">
            <Link href="/about" className="text-xs uppercase tracking-widest text-muted-foreground hover:text-bronze transition-colors">About</Link>
            <Link href="https://github.com/ValenteCreativo/Side-B" target="_blank" rel="noopener noreferrer" className="text-xs uppercase tracking-widest text-muted-foreground hover:text-bronze transition-colors">GitHub</Link>
          </div>
        </div>
      </footer>
    </div>
...
function FeatureCard({ icon, title, description, link }: { icon: React.ReactNode, title: string, description: string, link: string }) {
  return (
    <Link href={link} className="group relative bg-background p-12 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-refined">
      <div className="mb-8 transition-transform duration-300 group-hover:scale-110">{icon}</div>
      <h3 className="text-2xl font-bold mb-4 tracking-tight uppercase group-hover:text-bronze transition-colors">{title}</h3>
